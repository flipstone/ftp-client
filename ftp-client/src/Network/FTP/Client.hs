...
data FTPResponse = FTPResponse {
    frStatus :: ResponseStatus, -- ^ Interpretation of the first AC.digit of an FTP response code
    frCode :: Int, -- ^ The three AC.digit response code
...
getResponse h = do
    ...
    Monad.when (C.length code < 3 || not (C.all AC.isDigit code))
        $ MIO.liftIO $ Exception.throwIO $ BadProtocolResponseException line
...
ensureSucessfulData h resp = do
    resp' <- case frStatus resp of
        Success -> do
            ...
            return newResp
        _ -> return resp
    MIO.liftIO $ Monad.when (frStatus resp' /= Wait)
        $ Exception.throwIO $ UnsuccessfulException resp
...
getAllLineResp h =
    let collect :: (MIO.MonadIO n, MonadCatch n) => [ByteString] -> n ByteString
        collect ret = ( do
            line <- MIO.liftIO $ getLineResp h
            collect (ret <> [line]))
...
parseResponse resp p =
    ...
    in case AC.parseOnly p parsableMessage of
        Right x -> return x
        Left _ -> MIO.liftIO $ Exception.throwIO
            $ BadProtocolResponseException parsableMessage

ensureCode :: MIO.MonadIO m => FTPResponse -> Int -> m ()
ensureCode resp code =
    MIO.liftIO $ Monad.when (frCode resp /= code)
        $ MIO.liftIO $ Exception.throwIO $ UnsuccessfulException resp
...
getMlsxResponse h =
    let collect :: (MIO.MonadIO n, MonadCatch n) => [MlsxResponse] -> n [MlsxResponse]
        collect ret = ( do
            line <- MIO.liftIO $ getLineResp h
            collect $
                if C.null line
